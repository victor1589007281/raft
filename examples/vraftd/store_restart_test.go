...
// requires every record to survive in order. It runs against the library
// filestore in both legacy and single-WAL v2 formats.
func TestFileLogStoreRestartNoOverwrite(t *testing.T) {
	for _, opts := range []filestore.Options{
		{}, // legacy
		{SingleWAL: true, SparseIndex: true, Codec: filestore.IndexLSNCodec{}}, // v2
	} {
		dir := t.TempDir()
		newStore := func() *filestore.Store {
			s, err := filestore.Open(dir, opts)
			if err != nil {
				t.Fatalf("open: %v", err)
			}
			return s
		...
		// Restart #1: reload then append two more records (the noop + command
		// that every leader emits after taking over).
		store = newStore()
		...
		if first != 1 || last != 52 {
			t.Fatalf("bad range after restarts (opts %+v): first=%d last=%d (want 1..52)", opts, first, last)
		}
		...
		// v2: the sparse index must cover every record after restarts.
		if opts.SingleWAL {
			if idx, ok := store.LookupByLSN(52); !ok || idx != 52 {
				t.Fatalf("LookupByLSN(52) after restarts = %d,%v", idx, ok)
			}
		}
		store.Close()
	}
...
func TestFileStableStorePersistsAcrossRestart(t *testing.T) {
	dir := t.TempDir()
	s1, err := filestore.OpenStable(dir)
	if err != nil {
		t.Fatalf("open: %v", err)
	}
	...
	s2, err := filestore.OpenStable(dir)
	if err != nil {
